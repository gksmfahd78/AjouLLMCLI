  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const err = new Error(`API request failed (${response.status}): ${detail}`);
        err.status = response.status;
        throw err;
      }
      return data;
    } catch (error) {
      if (timed.didTimeout()) { lastError = new Error(`API request timed out after ${getRequestTimeoutMs()}ms`); break; }
      lastError = error;
      if (signal?.aborted || (error.status >= 400 && error.status < 500)) break;
...
function buildMessages(systemPrompt, history, prompt, reviewGuidance = "") {
  const messages = [];
  const systemParts = [FINAL_REASONING_PROMPT];
  ...
  const context = (Array.isArray(messages) ? messages : []).slice(-12).map((message) => {
    const content = typeof message.content === "string"
      ...
      : Array.isArray(message.content)
        ...
        : JSON.stringify(message.tool_calls || "").slice(0, 1500);
...
async function requestInternalReview(config, messages, signal) {
  try {
    const data = await requestJson(`${config.baseUrl}/chat/completions/`, {
      ...
      body: JSON.stringify({
        model: config.reviewModel || INTERNAL_REVIEW_MODEL,
        messages,
        temperature: 0,
        top_p: 1,
    ...
    throw new Error(`Internal review failed with ${config.reviewModel || INTERNAL_REVIEW_MODEL}: ${error.message || String(error)}`);
  }
}
...
async function _streamSse(config, history, prompt, onToken, signal, imageUrls = []) {
  const reviewGuidance = config.skipReview ? "" : await buildInternalReview(config, history, prompt, signal);
  const timed = withTimeoutSignal(signal);
  try {
    const response = await fetch(`${config.baseUrl}/chat/completions/`, {
      method: "POST",
      headers: createHeaders(config.apiKey),
      body: JSON.stringify(buildRequestBody(config, history, buildUserContent(prompt, imageUrls), true, reviewGuidance)),
      signal: timed.signal
    });
...
async function streamChat(config, history, prompt, imageUrls = []) {
  const text = await _streamSse(config, history, prompt, (token) => process.stdout.write(token), undefined, imageUrls);
  process.stdout.write("\n");
  return text;
}

async function streamChatTui(config, history, prompt, onToken, signal, imageUrls = []) {
  return _streamSse(config, history, prompt, (_token, full) => onToken(full), signal, imageUrls);
}

async function runChatOnce(config, prompt, history = [], imageUrls = []) {
  ensureApiKey(config);
  if (config.stream) {
    const text = await streamChat(config, history, prompt, imageUrls);
    return { text };
  }
  const reviewGuidance = config.skipReview ? "" : await buildInternalReview(config, history, prompt);
  const data = await requestJson(`${config.baseUrl}/chat/completions/`, {
    method: "POST",
    headers: createHeaders(config.apiKey),
    body: JSON.stringify(buildRequestBody(config, history, buildUserContent(prompt, imageUrls), false, reviewGuidance))
  });
...
module.exports = {
  createHeaders, ensureApiKey, requestJson, extractContent, buildUserContent,
