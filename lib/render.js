...
function buildInputBox(state, width) {
  ...
  const hintText = state.busy
    ...
    : color("2", "  Enter  ·  Ctrl+N newline  ·  ↑↓ history  ·  ←→ cursor  ·  Ctrl+←→ word  ·  /help");
...
let _renderInputTimer = null;

function scheduleRenderInput(state) {
  if (_renderTimer) return;
  if (_renderInputTimer) return;
  _renderInputTimer = setTimeout(() => { _renderInputTimer = null; renderInputOnly(state); }, 8);
}

function scheduleRender(state) {
  if (_renderInputTimer) { clearTimeout(_renderInputTimer); _renderInputTimer = null; }
  if (_renderTimer) return;
...
module.exports = {
  ...
  renderInputOnly, scheduleRender, scheduleRenderInput, renderTui, logTui
};
