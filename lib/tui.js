...
const { ensureCacheDir, writeWorkspaceFile, readTextFile, listProjectFiles, invalidateProjectFileCache } = require("./workspace");
const { loadConfig, saveConfig, getRuntimeConfig } = require("./config");
const { requestJson, createHeaders, fetchCredits, runChatOnce, streamChatTui } = require("./api");
...
const { runAgentLoop, buildAgentEditsWithRetry, applyAgentEdits } = require("./agent");
const { renderTui, logTui, scheduleRender, inputBoxHeight, color, wrapLine } = require("./render");
...
async function compactHistory(state) {
  ...
  renderTui(state);
  const transcript = state.history.map((m) => `${m.role}: ${String(m.content).slice(0, 500)}`).join("\n");
  const summaryConfig = { ...state.config, model: UTILITY_MODEL, stream: false, maxTokens: 2048, systemPrompt: "Summarize concisely in English only." };
  try {
...
async function executeTuiInput(state, line) {
  ...
  if (prompt.startsWith("/")) {
    ...
    if (command === "export") {
      ...
      const lines = state.history.map((m) => `**${m.role}:**\n\n${m.content}`).join("\n\n---\n\n");
  ...
  try {
    if (state.mode === "chat") {
      ...
      let lastRender = 0;
      const fullText = await streamChatTui(state.config, state.history, prompt, (accumulated) => {
      ...
      const plan = state.config.apiKey
        ? await buildModelBackedPlan(prompt, ranked, { ...state.config, stream: false })
      ...
      const resolvedPrompt = resolveAtMentions(prompt);
      const agentMessages = [
      ...
      state.abortController = ac;
      const undoBatch = [];
      function permissionDesc(toolName, args) {
      ...
      if (!state.config.apiKey) { logTui(state, "error", "No API key configured."); return; }
      logTui(state, "agent", `Scanning project for: ${prompt}`);
      renderTui(state);
      const ranked = rankFilesForTask(prompt, 10);
      state.lastFiles = ranked.ranked.map((item) => item.entry.path);
      logTui(state, "agent", `Context: ${ranked.ranked.map((item) => item.entry.path).join(", ") || "(none)"}`);
      renderTui(state);
      const editSpec = await buildAgentEditsWithRetry(
        prompt, ranked, { ...state.config, stream: false },
        (msg) => { logTui(state, "agent", msg); renderTui(state); }
      );
      const changedFiles = applyAgentEdits(editSpec, false);
      state.lastFiles = changedFiles;
      state.changeSummary = { count: changedFiles.length, files: changedFiles, action: "Review changes" };
      const summary = [
        editSpec.summary || "",
        "",
        ...(editSpec.plan?.length ? ["Plan:", ...editSpec.plan.map((s) => `- ${s}`), ""] : []),
        `Written files (${changedFiles.length}):`,
        ...changedFiles.map((f) => `- ${f}`)
      ].filter((l, i, arr) => l || (i > 0 && arr[i - 1])).join("\n");
      logTui(state, "agent", summary);
    }
  } catch (error) {
...
async function promptLoop() {
  ...
  const state = {
    ...
    logs: (() => {
      const session = loadTuiSession();
      const msgs = [
        { role: "system", text: "AjouLLM Studio started." },
        { role: "system", text: "Default mode is chat. Switch with /mode agent for file editing." },
        { role: "system", text: loadClaudeMd() ? `ajoullm.md loaded from ${process.cwd()}` : "Tip: create ajoullm.md for project-specific instructions." }
      ];
      if (session?.history?.length) msgs.push({ role: "system", text: `Session restored (${session.history.length / 2 | 0} turns). /clear to reset.` });
      return msgs;
    })(),
    logSeq: 1,
    lastFiles: [],
  ...
  const onKeypress = async (str, key) => {
    ...
    if (key?.name === "up") {
      ...
      resetTabState(); scheduleRender(state); return;
    }
    if (key?.name === "down") {
    ...
    if (key?.name === "left") { state.cursorPos = Math.max(0, state.cursorPos - 1); resetTabState(); scheduleRender(state); return; }
    if (key?.name === "right") { state.cursorPos = Math.min(state.input.length, state.cursorPos + 1); resetTabState(); scheduleRender(state); return; }
    if (key?.name === "home" || (key?.ctrl && key.name === "a")) { state.cursorPos = 0; resetTabState(); scheduleRender(state); return; }
    if (key?.name === "end" || (key?.ctrl && key.name === "e")) { state.cursorPos = state.input.length; resetTabState(); scheduleRender(state); return; }
    ...
    if (key?.name === "tab") {
      ...
      if (!tabSeed || tabSeed.completions.length === 0) { resetTabState(); scheduleRender(state); return; }
