...
const { CACHE_DIR, CACHE_PATH, AGENT_SESSION_PATH, TEXT_FILE_EXTENSIONS, PROJECT_FILE_CACHE_TTL_MS } = require("./paths");
...
function readTextFile(relPath, maxChars = 12000) {
  ...
  return fs.readFileSync(absPath, "utf8").replace(/\0/g, "").slice(0, maxChars);
}
...
function matchGlob(pattern, filePath) {
  const re = new RegExp(
    "^" + pattern.replace(/[.+^${}()|[\]\\]/g, "\\$&").replace(/\*\*/g, "\x00").replace(/\*/g, "[^/]*").replace(/\x00/g, ".*") + "$"
  );
...
module.exports = {
  ...
  isLikelyTextFile, readTextFile, writeWorkspaceFile, getFileStats, matchGlob
};
